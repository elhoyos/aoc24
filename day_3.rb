...
# input = "do()#{input.chomp}don't()"

# conditioned_added_mul = input.scan(/(?<=do\(\)).*?(?:mul\((\d{1,3}),(\d{1,3})\))+.*?(?=don't)/).sum { _1.map(&:to_i).reduce(&:*) }
